...
| **GIS workflow alignment** | 50% | 68% | The largest move, and the one Vivek's question targeted: village-level choropleth, spatial selection, spatial comparison, and locate-from-queue now match how a QGIS-native team works **[S1, S2]**. Scored well below the interaction model's merit because of a data finding made during verification — **100% of the 1403 village boundaries in the database are 5-point axis-aligned rectangles** (`ST_NPoints = 5` for every row), i.e. bounding boxes, not real village outlines. The map therefore renders a grid of rectangles. To a GIS-native viewer this is the first thing visible, and it caps how far this dimension can honestly be scored regardless of interaction quality. Also still missing: canal-command layer, DEM/slope, LULC overlay (Jaltol integrates 10 m IndiaSAT LULC **[S3]**), and district-wide rendering of unmonitored villages. |
...
> **Correction.** An earlier draft of this document scored GIS workflow at 80% and the overall at 70%. That was written before checking the geometry actually stored. Measuring it (`ST_NPoints` over all 1403 village rows) showed every boundary is a rectangle, which is disqualifying for a claim of GIS-workflow parity. The score is corrected down rather than left standing.
...
**The binding constraint is now data, not presentation.** The interaction model is in place; what limits it is that the underlying village geometry is placeholder bounding boxes. Replacing that with real village outlines is a data-pipeline change (`fetch_raichur_villages_osm.py`), deliberately outside this presentation-only redesign — but it is the single highest-value next step, and until it is done the map understates the work behind it.
